    ...
    <div id="chat">
        ...
        <div class="main">
            <div class="m-message" v-scroll-bottom="sessionNow.messages">
                <ul>
                    <li style="text-align: center;color: blue;cursor: pointer;">点击查看历史记录</li>
...
var now = new Date()
    var app = new Vue({
        ...
        created:function(){
            var my=this;
            socket.on('witer',function(obj){
                obj.self=false;
                for (var i = 0; i < my.userList.length; i++) {
                    // my.userList[i].id=obj.userid.id;
                    if (my.userList[i].id==obj.userid.id) {
                        var mag={   
                            img:obj.userid.img,
                            text: obj.text,
                            date: obj.date
                        }
                        my.sessionList[i].messages.push(mag);
                        return;
                    }
                    
                }
                my.userList.push(obj.userid);
                var session={
